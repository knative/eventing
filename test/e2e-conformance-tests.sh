...
echo "Running Conformance tests for: Multi Tenant Channel Based Broker (v1), Channel (v1), InMemoryChannel (v1) , ApiServerSource (v1), ContainerSource (v1) and PingSource (v1)"
go_test_e2e -timeout=30m -parallel=12 ./test/conformance \
  ...
  -sources=sources.knative.dev/v1:PingSource,sources.knative.dev/v1:ApiServerSource,sources.knative.dev/v1:ContainerSource \
  || fail_test
