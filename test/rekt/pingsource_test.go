...
func TestPingSourceDataPlaneLabels(t *testing.T) {
	t.Parallel()
	...
	srcname := feature.MakeRandomK8sName("pingsource")
	env.Prerequisite(ctx, t, pingsource.GoesReady(srcname))
	...
	env.Test(ctx, t, workloads.Selector(workloads.Workload{
		...
		PartialName: "pingsource-mt-adapter",
