...
func addressableAllowsAuthorizedRequest(gvr schema.GroupVersionResource, kind, name string, inputEventEncoding cloudevents.Encoding) *feature.Feature {
	...
	f.Alpha(kind).
		...
		Must("get 202 or 204 on response", eventassert.OnStore(source).Match(eventassert.OneOf(eventassert.MatchStatusCode(202), eventassert.MatchStatusCode(204))).AtLeast(1))
...
func addressableRespectsEventPolicyFilters(gvr schema.GroupVersionResource, kind, name string, inputEventEncoding cloudevents.Encoding) *feature.Feature {
	...
	f.Alpha(kind).
		...
		Must("get 202 or 204 on response", eventassert.OnStore(source1).Match(eventassert.OneOf(eventassert.MatchStatusCode(202), eventassert.MatchStatusCode(204))).AtLeast(1))
