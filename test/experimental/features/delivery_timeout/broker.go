...
// BrokerToSink tests a scenario where the flow is source -> broker -> sink (timeout) -- fallback to -> dead letter sink
func BrokerToSink(brokerOpts ...manifest.CfgFn) *feature.Feature {
	...
	triggerName := feature.MakeRandomK8sName("trigger-sink")
	...
	f.Setup("Install broker", broker.Install(brokerName, brokerOpts...))
	f.Setup("Broker is ready", broker.IsReady(brokerName))

	f.Setup("Install broker -> sink subscription", func(ctx context.Context, t feature.T) {
		namespace := environment.FromContext(ctx).Namespace()
		_, err := eventingclient.Get(ctx).EventingV1().Triggers(namespace).Create(ctx,
			&eventingv1.Trigger{
