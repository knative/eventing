...
import (
	...
	duckv1beta1 "knative.dev/eventing/vendor/knative.dev/pkg/apis/duck/v1beta1"
...
func getSourceDuckTypeSupportedVersion(sourceName string, client *testlib.Client, source *metav1.TypeMeta) (string, error) {
	metaResource := resources.NewMetaResource(sourceName, client.Namespace, source)
	obj, err := duck.GetGenericObject(client.Dynamic, metaResource, &duckv1beta1.Source{})
	if err != nil {
		return "", errors.Wrapf(err, "Unable to GET the channel %v", metaResource)
	}
	sourceDuck, ok := obj.(*duckv1beta1.Source)
	if !ok {
		return "", errors.Wrapf(err, "Unable to cast the channel %v", metaResource)
	}
	return sourceDuck.ObjectMeta.Annotations[messaging.SubscribableDuckVersionAnnotation], nil
}
