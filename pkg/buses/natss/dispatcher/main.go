...
func main() {
	...
	logger = logger.With(
		...
		zap.String("channels.knative.dev/busType", natss.BusType),
	...
	bus, err := natss.NewNatssBusDispatcher(ref, natss.SetNewBusDispatcher(opts))
	if err != nil {
		logger.Fatalf("Error starting natss bus dispatcher: %v", err)
	}

	// set up signals so we handle the first shutdown signal gracefully
	stopCh := signals.SetupSignalHandler()
	bus.Run(threadsPerReconciler, stopCh, "knative-dispatcher-natss")
}
