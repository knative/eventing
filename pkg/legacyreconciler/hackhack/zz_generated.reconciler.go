...
import (
	...
	legacysourcesv1alpha1 "knative.dev/eventing/pkg/legacyclient/listers/legacysources/v1alpha1"
...
type Core struct {
	...
	Lister legacysourcesv1alpha1.ContainerSourceLister
...
func (r *Core) updateStatus(existing *v1alpha1.ContainerSource, desired *v1alpha1.ContainerSource) error {
	...
	return RetryUpdateConflicts(func(attempts int) (err error) {
		...
		if attempts > 0 {
			existing, err = r.Client.LegacysourcesV1alpha1().ContainerSources(desired.Namespace).Get(desired.Name, metav1.GetOptions{})
			if err != nil {
				return err
			}
		}
		...
		_, err = r.Client.LegacysourcesV1alpha1().ContainerSources(existing.Namespace).UpdateStatus(existing)
		return err
	})
}
...
func (r *Core) updateFinalizers(ctx context.Context, desired *v1alpha1.ContainerSource) (*v1alpha1.ContainerSource, bool, error) {
	...
	update, err := r.Client.LegacysourcesV1alpha1().ContainerSources(desired.Namespace).Patch(existing.Name, types.MergePatchType, patch)
