...
type HandlerBinding struct {
	...
	receiver   *channel.EventReceiverBindings
	dispatcher *channel.EventDispatcherBinding
...
func NewHandlerBinding(ctx context.Context, logger *zap.Logger, config Config) (*HandlerBinding, error) {
	handler := &HandlerBinding{
		logger:     logger,
		config:     config,
		dispatcher: channel.NewEventDispatcherBindingFromConfig(logger, config.DispatcherConfig),
		timeout:    defaultTimeout,
	}
	// The receiver function needs to point back at the handler itself, so set it up after
	// initialization.
	receiver, err := channel.NewEventReceiverBinding(ctx, createReceiverFunctionBinding(handler), logger)
	if err != nil {
		return nil, err
	}
...
func createReceiverFunctionBinding(f *HandlerBinding) func(context.Context, channel.ChannelReference, binding.Message, nethttp.Header) error {
...
func (f *HandlerBinding) ServeHTTP(response nethttp.ResponseWriter, request *nethttp.Request) {
...
func (f *HandlerBinding) dispatch(ctx context.Context, message binding.Message, additionalHeaders nethttp.Header) error {
...
func (f *HandlerBinding) makeFanoutRequest(ctx context.Context, message binding.Message, additionalHeaders nethttp.Header, sub eventingduck.SubscriberSpec) error {
	return f.dispatcher.DispatchEventWithDelivery(ctx, message, additionalHeaders, sub.SubscriberURI.String(), sub.ReplyURI.String(), &channel.DeliveryOptions{DeadLetterSink: sub.DeadLetterSinkURI.String()})
}
