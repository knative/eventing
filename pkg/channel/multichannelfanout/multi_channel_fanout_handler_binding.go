...
type HandlerBinding struct {
	logger   *zap.Logger
	handlers map[string]*fanout.HandlerBinding
	config   Config
}

// NewHandler creates a new Handler.
func NewHandlerBinding(ctx context.Context, logger *zap.Logger, conf Config) (*HandlerBinding, error) {
	handlers := make(map[string]*fanout.HandlerBinding, len(conf.ChannelConfigs))

	for _, cc := range conf.ChannelConfigs {
		key := makeChannelKeyFromConfig(cc)
		handler, err := fanout.NewHandlerBinding(ctx, logger, cc.FanoutConfig)
		if err != nil {
...
func (h *HandlerBinding) ConfigDiff(updated Config) string {
...
func (h *HandlerBinding) CopyWithNewConfig(ctx context.Context, conf Config) (*HandlerBinding, error) {
	return NewHandlerBinding(ctx, h.logger, conf)
}
