...
func TestNewHandlerBinding(t *testing.T) {
	testCases := []struct {
		name      string
		config    Config
		createErr string
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			_, err := NewHandlerBinding(context.TODO(), zap.NewNop(), tc.config)
			if tc.createErr != "" {
				if err == nil {
...
func TestCopyHandlerBindingWithNewConfig(t *testing.T) {
	orig := Config{
		ChannelConfigs: []ChannelConfig{
			{
				Namespace: "default",
	...
	h, err := NewHandlerBinding(context.TODO(), zap.NewNop(), orig)
	if err != nil {
...
func TestConfigDiffHandlerBinding(t *testing.T) {
	config := Config{
		ChannelConfigs: []ChannelConfig{
			{
				Namespace: "default",
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			h, err := NewHandlerBinding(context.TODO(), zap.NewNop(), tc.orig)
			if err != nil {
				t.Errorf("Unable to create handler: %v", err)
			}
...
func TestServeHTTPHandlerBinding(t *testing.T) {
	...
	for n, tc := range testCases {
		t.Run(n, func(t *testing.T) {
			...
			h, err := NewHandlerBinding(context.TODO(), zap.NewNop(), tc.config)
			if err != nil {
