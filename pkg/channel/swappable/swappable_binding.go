...
func NewHandlerBinding(handler *multichannelfanout.HandlerBinding, logger *zap.Logger) *HandlerBinding {
	h := &HandlerBinding{
...
func NewEmptyHandlerBinding(context context.Context, logger *zap.Logger) (*HandlerBinding, error) {
	h, err := multichannelfanout.NewHandlerBinding(context, logger, multichannelfanout.Config{})
	if err != nil {
		return nil, err
	}
	return NewHandlerBinding(h, logger), nil
}

// getMultiChannelFanoutHandler gets the current multichannelfanout.Handler to delegate all HTTP
// requests to.
func (h *HandlerBinding) getMultiChannelFanoutHandler() *multichannelfanout.HandlerBinding {
	return h.fanout.Load().(*multichannelfanout.HandlerBinding)
}

// setMultiChannelFanoutHandler sets a new multichannelfanout.Handler to delegate all subsequent
// HTTP requests to.
func (h *HandlerBinding) setMultiChannelFanoutHandler(nh *multichannelfanout.HandlerBinding) {
	h.fanout.Store(nh)
}
...
// HTTP traffic.
func (h *HandlerBinding) UpdateConfig(context context.Context, config *multichannelfanout.Config) error {
	if config == nil {
...
func (h *HandlerBinding) ServeHTTP(response http.ResponseWriter, request *http.Request) {
