...
func TestHandlerBinding(t *testing.T) {
	...
	for n, tc := range testCases {
		t.Run(n, func(t *testing.T) {
			h, err := NewEmptyHandlerBinding(context.TODO(), zap.NewNop())
			if err != nil {
...
func TestHandlerBinding_InvalidConfigChange(t *testing.T) {
	testCases := map[string]struct {
...
func TestHandlerBinding_NilConfigChange(t *testing.T) {
	h, err := NewEmptyHandlerBinding(context.TODO(), zap.NewNop())
	if err != nil {
		t.Errorf("Unexpected error creating handler: %v", err)
	}
...
func updateConfigAndTestBinding(t *testing.T, h *HandlerBinding, config multichannelfanout.Config) {
	server := httptest.NewServer(&successHandler{})
	defer server.Close()
...
func assertRequestBindingAccepted(t *testing.T, h *HandlerBinding) {
