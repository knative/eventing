...
func (c *client) Send(ctx context.Context, event cloudevents.Event) error {
	c.applyOverrides(ctx, &event)
	err := c.ceClient.Send(ctx, event)
	err = c.reportCount(ctx, event, err)

	return err
}

// Request implements client.Request
func (c *client) Request(ctx context.Context, event cloudevents.Event) (*cloudevents.Event, error) {
	c.applyOverrides(ctx, &event)
	resp, err := c.ceClient.Request(ctx, event)
	err = c.reportCount(ctx, event, err)
