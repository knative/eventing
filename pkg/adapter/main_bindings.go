...
type AdapterBindings interface {
	Start(stopCh <-chan struct{}) error
}

type AdapterBindingsConstructor func(ctx context.Context, env EnvConfigAccessor, client *kncloudevents.HttpBindingSender, reporter source.StatsReporter) AdapterBindings

func MainBindings(component string, ector EnvConfigConstructor, ctor AdapterBindingsConstructor) {
	MainBindingsWithContext(signals.NewContext(), component, ector, ctor)
}

func MainBindingsWithContext(ctx context.Context, component string, ector EnvConfigConstructor, ctor AdapterBindingsConstructor) {
	flag.Parse()
	...
	httpBindingsSender, err := kncloudevents.NewHttpBindingsSender(nil, env.GetSinkURI())
	if err != nil {
	...
	logger.Info("Starting Receive AdapterBindings", zap.Any("adapter", adapter))
