	...
	table := TableTest{
		{
			...
			Name: "Trigger Channel endpoints unavailable",
			Key:  testKey,
			Objects: []runtime.Object{
				NewBroker(brokerName, testNS,
					WithBrokerClass(eventing.MTChannelBrokerClassValue),
			...
			WantStatusUpdates: []clientgotesting.UpdateActionImpl{{
				Object: NewBroker(brokerName, testNS,
					...
					WithFilterFailed("EndpointSlicesUnavailable", "EndpointSlices are unavailable."),
			...
			Objects: []runtime.Object{
				...
				createChannel(withChannelReady),
				imcConfigMap(),
				NewEndpointSlice(filterServiceName, systemNS,
					WithEndpointSliceLabels(withServiceNameLabel(FilterLabels(), filterServiceName)),
					WithEndpointSliceAddresses(readyEndpoint("127.0.0.1"))),
				NewEndpointSlice(ingressServiceName, systemNS,
					WithEndpointSliceLabels(withServiceNameLabel(IngressLabels(), ingressServiceName)),
					WithEndpointSliceAddresses(readyEndpoint("127.0.0.1"))),
			},
			WantStatusUpdates: []clientgotesting.UpdateActionImpl{{
				Object: NewBroker(brokerName, testNS,
...
func withServiceNameLabel(labels map[string]string, serviceName string) map[string]string {
	merged := make(map[string]string, len(labels)+1)
	for k, v := range labels {
		merged[k] = v
	}
	merged[discoveryv1.LabelServiceName] = serviceName
	return merged
}
