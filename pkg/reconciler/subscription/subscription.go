...
func (r *Reconciler) getSubStatus(subscription *v1.Subscription, channel *eventingduckv1alpha1.ChannelableCombined) (eventingduckv1.SubscriberStatus, error) {
...
func (r *Reconciler) getSubStatusV1Alpha1(subscription *v1.Subscription, channel *eventingduckv1alpha1.ChannelableCombined) (eventingduckv1.SubscriberStatus, error) {
	...
	for _, sub := range subscribableStatus.Subscribersv1 {
	...
	for i, v := range channel.Spec.Subscribers {
		if v.UID == sub.UID {
			...
			if sub.Status.PhysicalSubscription.DeadLetterSinkURI != nil {
				channel.Spec.Subscribers[i].Delivery = &eventingduckv1.DeliverySpec{
