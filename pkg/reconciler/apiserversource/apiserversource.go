...
func (r *Reconciler) reconcileEventTypes(ctx context.Context, src *v1alpha1.ApiServerSource) error {
	...
	for _, eventType := range toDelete {
		if err = r.Core.Client.EventingV1alpha1().EventTypes(src.Namespace).Delete(eventType.Name, &metav1.DeleteOptions{}); err != nil {
	...
	for _, eventType := range toCreate {
		if _, err = r.Core.Client.EventingV1alpha1().EventTypes(src.Namespace).Create(&eventType); err != nil {
