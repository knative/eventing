...
type EventTypeLister interface {
	...
	List(selector labels.Selector) (ret []*v1.EventType, err error)
...
// List lists all EventTypes in the indexer.
func (s *eventTypeLister) List(selector labels.Selector) (ret []*v1.EventType, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.EventType))
	})
	return ret, err
}
...
type EventTypeNamespaceLister interface {
	...
	Get(name string) (*v1.EventType, error)
...
// List lists all EventTypes in the indexer for a given namespace.
func (s eventTypeNamespaceLister) List(selector labels.Selector) (ret []*v1.EventType, err error) {
...
// Get retrieves the EventType from the indexer for a given namespace and name.
func (s eventTypeNamespaceLister) Get(name string) (*v1.EventType, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("eventtype"), name)
	}
	return obj.(*v1.EventType), nil
}
