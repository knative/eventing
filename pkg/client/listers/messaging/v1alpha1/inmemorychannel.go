...
// List lists all InMemoryChannels in the indexer.
func (s *inMemoryChannelLister) List(selector labels.Selector) (ret []*v1alpha1.InMemoryChannel, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.InMemoryChannel))
	})
	return ret, err
}
...
// List lists all InMemoryChannels in the indexer for a given namespace.
func (s inMemoryChannelNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.InMemoryChannel, err error) {
...
// Get retrieves the InMemoryChannel from the indexer for a given namespace and name.
func (s inMemoryChannelNamespaceLister) Get(name string) (*v1alpha1.InMemoryChannel, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("inmemorychannel"), name)
	}
	return obj.(*v1alpha1.InMemoryChannel), nil
}
