...
// controller reconciling v1alpha1.Parallel.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.Parallel. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha1.Parallel) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.Parallel.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.Parallel. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.Parallel) reconciler.Event
}

// reconcilerImpl implements controller.Reconciler for v1alpha1.Parallel resources.
type reconcilerImpl struct {
	// Client is used to write back status updates.
	Client versioned.Interface

	// Listers index properties about resources
	Lister flowsv1alpha1.ParallelLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister flowsv1alpha1.ParallelLister, recorder record.EventRecorder, r Interface) controller.Reconciler {
...
func (r *reconcilerImpl) updateStatus(existing *v1alpha1.Parallel, desired *v1alpha1.Parallel) error {
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.Parallel) error {
	...
	_, err = r.Client.FlowsV1alpha1().Parallels(resource.Namespace).Patch(resource.Name, types.MergePatchType, patch)
	if err != nil {
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.Parallel) error {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.Parallel, reconcileEvent reconciler.Event) error {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
