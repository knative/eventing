...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister sourcesv1alpha1.ApiServerSourceLister, recorder record.EventRecorder, r Interface) controller.Reconciler {
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.ApiServerSource) error {
	finalizerName := defaultFinalizerName

	actual, err := r.Lister.ApiServerSources(resource.Namespace).Get(resource.Name)
	if err != nil {
		return err
	}
	...
	_, err = r.Client.SourcesV1alpha1().ApiServerSources(resource.Namespace).Patch(resource.Name, types.MergePatchType, patch)
	if err != nil {
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.ApiServerSource) error {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.ApiServerSource, reconcileEvent reconciler.Event) error {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
