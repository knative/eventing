...
func (r *reconcilerImpl) updateStatus(existing *v1alpha2.PingSource, desired *v1alpha2.PingSource) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {
			existing, err = r.Client.SourcesV1alpha2().PingSources(desired.Namespace).Get(desired.Name, metav1.GetOptions{})
			if err != nil {
				return err
			}
		}
		...
		_, err = r.Client.SourcesV1alpha2().PingSources(existing.Namespace).UpdateStatus(existing)
		return err
	})
}
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha2.PingSource) (*v1alpha2.PingSource, error) {
	...
	resource, err = r.Client.SourcesV1alpha2().PingSources(resource.Namespace).Patch(resource.Name, types.MergePatchType, patch)
	if err != nil {
