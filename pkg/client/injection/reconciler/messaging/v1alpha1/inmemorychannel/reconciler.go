...
// controller reconciling v1alpha1.InMemoryChannel.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.InMemoryChannel. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha1.InMemoryChannel) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.InMemoryChannel.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.InMemoryChannel. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.InMemoryChannel) reconciler.Event
}

// reconcilerImpl implements controller.Reconciler for v1alpha1.InMemoryChannel resources.
type reconcilerImpl struct {
	// Client is used to write back status updates.
	Client versioned.Interface

	// Listers index properties about resources
	Lister messagingv1alpha1.InMemoryChannelLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister messagingv1alpha1.InMemoryChannelLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	original, err := r.Lister.InMemoryChannels(namespace).Get(name)
...
func (r *reconcilerImpl) updateStatus(existing *v1alpha1.InMemoryChannel, desired *v1alpha1.InMemoryChannel) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {
			existing, err = r.Client.MessagingV1alpha1().InMemoryChannels(desired.Namespace).Get(desired.Name, metav1.GetOptions{})
			if err != nil {
				return err
			}
		}
		...
		_, err = r.Client.MessagingV1alpha1().InMemoryChannels(existing.Namespace).UpdateStatus(existing)
		return err
	})
}
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.InMemoryChannel) (*v1alpha1.InMemoryChannel, error) {
	finalizerName := defaultFinalizerName

	actual, err := r.Lister.InMemoryChannels(resource.Namespace).Get(resource.Name)
	if err != nil {
		return resource, err
	}
	...
	resource, err = r.Client.MessagingV1alpha1().InMemoryChannels(resource.Namespace).Patch(resource.Name, types.MergePatchType, patch)
	if err != nil {
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.InMemoryChannel) (*v1alpha1.InMemoryChannel, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.InMemoryChannel, reconcileEvent reconciler.Event) (*v1alpha1.InMemoryChannel, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
