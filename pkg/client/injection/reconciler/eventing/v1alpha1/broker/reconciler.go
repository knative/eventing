...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister eventingv1alpha1.BrokerLister, recorder record.EventRecorder, r Interface) controller.Reconciler {
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.Broker) error {
	...
	_, err = r.Client.EventingV1alpha1().Brokers(resource.Namespace).Patch(resource.Name, types.MergePatchType, patch)
	if err != nil {
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.Broker) error {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.Broker, reconcileEvent reconciler.Event) error {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
