...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	original, err := r.Lister.ConfigMapPropagations(namespace).Get(name)
...
func (r *reconcilerImpl) updateStatus(existing *v1alpha1.ConfigMapPropagation, desired *v1alpha1.ConfigMapPropagation) error {
	...
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		...
		if attempts > 0 {
			existing, err = r.Client.ConfigsV1alpha1().ConfigMapPropagations(desired.Namespace).Get(desired.Name, metav1.GetOptions{})
			if err != nil {
				return err
			}
		}
		...
		_, err = r.Client.ConfigsV1alpha1().ConfigMapPropagations(existing.Namespace).UpdateStatus(existing)
		return err
	})
}
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.ConfigMapPropagation) (*v1alpha1.ConfigMapPropagation, error) {
	...
	actual, err := r.Lister.ConfigMapPropagations(resource.Namespace).Get(resource.Name)
	if err != nil {
		return resource, err
	}
	...
	resource, err = r.Client.ConfigsV1alpha1().ConfigMapPropagations(resource.Namespace).Patch(resource.Name, types.MergePatchType, patch)
	if err != nil {
