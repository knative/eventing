...
func Get(ctx context.Context, selector string) v1.ApiServerSourceInformer {
	untyped := ctx.Value(Key{Selector: selector})
	if untyped == nil {
		logging.FromContext(ctx).Panicf(
			"Unable to fetch knative.dev/eventing/pkg/client/informers/externalversions/sources/v1.ApiServerSourceInformer with selector %s from context.", selector)
	}
