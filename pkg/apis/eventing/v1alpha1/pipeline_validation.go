...
func (t *Pipeline) CheckImmutableFields(ctx context.Context, og apis.Immutable) *apis.FieldError {
	if og == nil {
		return nil
	}

	original, ok := og.(*Pipeline)
	if !ok {
		return &apis.FieldError{Message: "The provided original was not a Pipeline"}
	}

	if diff, err := kmp.ShortDiff(original.Spec.Broker, t.Spec.Broker); err != nil {
		return &apis.FieldError{
			Message: "Failed to diff Pipeline",
			Paths:   []string{"spec"},
			Details: err.Error(),
		}
	} else if diff != "" {
