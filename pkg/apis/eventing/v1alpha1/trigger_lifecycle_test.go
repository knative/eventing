...
func TestTriggerAnnotateUserInfo(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			user:       u1,
			this:       &Trigger{Spec: TriggerSpec{Broker: defaultBroker, Filter: defaultTriggerFilter, StructuredEncoding: binaryEncoding}},
			prev:       &Trigger{Spec: TriggerSpec{Broker: defaultBroker, Filter: defaultTriggerFilter, StructuredEncoding: binaryEncoding}},
			wantedAnns: map[string]string{},
		}, {
			name: "update trigger which has annotations without diff",
			user: u2,
			this: withUserAnns(u1, u1, &Trigger{Spec: TriggerSpec{Broker: defaultBroker, Filter: defaultTriggerFilter, StructuredEncoding: binaryEncoding}}),
			prev: withUserAnns(u1, u1, &Trigger{Spec: TriggerSpec{Broker: defaultBroker, Filter: defaultTriggerFilter, StructuredEncoding: binaryEncoding}}),
			wantedAnns: map[string]string{
				eventing.CreatorAnnotation: u1,
				eventing.UpdaterAnnotation: u1,
			},
