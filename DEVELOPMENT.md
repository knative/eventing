...
- [`kubectl sniff`](https://github.com/eldadru/ksniff) to deploy and collect `tcpdump`
- (Optional) [Wireshark](https://www.wireshark.org/) to analyze the `tcpdump` output

After you installed all these tools, change the base image ko uses to build Eventing component images changing the [.ko.yaml](./.ko.yaml). 
You need an image that has the `tar` tool installed, for example:
...
Changing `<POD_NAME>` with the pod name of the component you wish to test, for example `imc-dispatcher-85797b44c8-gllnx`. 
This command will dump the `tcpdump` output with all the sniffed packets to `out.dump`. Then, you can open this file with Wireshark using:
