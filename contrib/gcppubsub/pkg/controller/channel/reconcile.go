...
func (r *reconciler) Finalize(ctx context.Context, obj eventingreconciler.ReconciledResource, recorder record.EventRecorder) error {
	...
	if err != nil {
		recorder.Eventf(c, v1.EventTypeWarning, subscriptionSyncFailed, "Failed to sync Subscription for the Channel: %v", err)
		return err
	}
	err = r.deleteTopic(ctx, originalPCS, gcpCreds)
	if err != nil {
		recorder.Eventf(c, v1.EventTypeWarning, topicDeleteFailed, "Failed to delete Topic for the Channel: %v", err)
		return err
	}
	return nil
}
