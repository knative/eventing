...
func (r *reconciler) ReconcileResource(ctx context.Context, obj eventingreconciler.ReconciledResource, recorder record.EventRecorder) (bool, reconcile.Result, error) {
	...
	return true, reconcile.Result{Requeue: requeue}, err
}
...
func (r *reconciler) reconcile(ctx context.Context, channel *eventingv1alpha1.Channel) (bool, error) {
	...
	if kafkaClusterAdmin == nil {
		...
		if err != nil {
			logger.Error(fmt.Sprintf("unable to build kafka admin client for %v", r.config), zap.Error(err))
			return false, err
		}
	}
	defer kafkaClusterAdmin.Close()

	if err := r.provisionChannel(channel, kafkaClusterAdmin, logger); err != nil {
...
func (r *reconciler) ShouldReconcile(_ context.Context, obj eventingreconciler.ReconciledResource, _ record.EventRecorder) bool {
	...
	if c.Spec.Provisioner != nil && c.Spec.Provisioner.Name == controller.Name {
		return true
	}
	return false
...
func (r *reconciler) OnDelete(ctx context.Context, obj eventingreconciler.ReconciledResource, recorder record.EventRecorder) error {
	...
	defer kafkaClusterAdmin.Close()
	if err := r.deprovisionChannel(channel, kafkaClusterAdmin, logger); err != nil {
		return err
	}
	return nil
}
...
func (r *reconciler) provisionChannel(channel *eventingv1alpha1.Channel, kafkaClusterAdmin sarama.ClusterAdmin, logger *zap.Logger) error {
...
func (r *reconciler) deprovisionChannel(channel *eventingv1alpha1.Channel, kafkaClusterAdmin sarama.ClusterAdmin, logger *zap.Logger) error {
	topicName := topicUtils.TopicName(controller.KafkaChannelSeparator, channel.Namespace, channel.Name)
...
func (r *reconciler) writeConfigMap(ctx context.Context, config *multichannelfanout.Config) error {
	logger := logging.FromContext(ctx).With(zap.Any("configMap", r.configMapKey))
